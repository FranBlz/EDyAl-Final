...
void tablahash_insertar(TablaHash *tabla, char *clave, ITree dato) {
  ...
  if (tabla->numElems + 1 <= tabla->capacidad) {
    ...
    if (tabla->tabla[idx].clave == NULL) {
      ...
      for (; tabla->tabla[aux % tabla->capacidad].clave != NULL;
           aux += incremento);

      tabla->tabla[aux % tabla->capacidad].clave =
          malloc(sizeof(char) * strlen(clave) + 1);
...
ITree tablahash_buscar(TablaHash * tabla, char *clave) {
  ...
  if (tabla->tabla[idx].clave != NULL) {
    CasillaHash *aux;
    int incremento = 1 + (tabla->hash(clave) % (tabla->capacidad));
    for (aux = &(tabla->tabla[idx % tabla->capacidad]);
         (aux->clave != NULL && (strcmp(aux->clave, clave) != 0))
         || (aux->dato == NULL && aux->clave != NULL);
         aux = &(tabla->tabla[(idx += incremento) % tabla->capacidad]));
